...
class HeaderBar extends React.Component {
    ...
    render() {
        return ( <
            ...
            /NavLink> { /* empty container found to be needed as a place holder for the burger meny contracted*/ } <
            ...
            span > < /span> <
            /a> <
            /div> { /* Contracted burger menu at start */ } <
            div className = "navbar-menu  navbar-dropdown" >
            <
            div className = "navbar-end" > { /* EACH ELEMENT IS ITERATED FROM THE OPTIONSARRAY */ } {
                this.state.optionsArray.map((option, ind) => {
                    return ( <
                        ...
                        onClick = { option.msg ? this.toggleMenu : this.state.logout } >
                        <
            ...
            /div> <
            /div> { /* Desktop menu, not visible on mobile*/ } <
            div className = "navbar-end is-hidden-touch"
            id = "desktop-menu" > { /* SINCE THERE IS NO MSG OPTION ON LOGOUT IT RENDERS DIFFERENT FUNCTIONS ON CLICK */ } <
            div id = "mainMenu"
            className = "dropdown is-right  is-fullheight"
            onMouseOver = {
                () => this.triggerMenu() }
            onMouseOut = {
                () => this.removeMenu() } >
            ...
            <
            button className = "button is-primary  is-fullheight"
            aria - haspopup = "true"
            ...
            role = "menu" >
            <
            div className = "dropdown-content" > { /* EACH ELEMENT IS ITERATED FROM THE OPTIONSARRAY */ } {
                this.state.optionsArray.map((option, ind) => {
                    return ( <
                        div key = { ind } > { /* SINCE THERE IS NO MSG OPTION ON LOGOUT IT RENDERS DIFFERENT FUNCTIONS ON CLICK */ } <
                        NavLink to = { option.path }
                        className = "dropdown-item"
                        onClick = { option.msg ? null : this.state.logout } >
                        <
                        div className = "" > { option.legend } < /div> <
                        div > { option.msg } < /div> <
                        /NavLink> {
